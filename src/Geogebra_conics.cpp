...
void Viewer_conic::remove_name_doublons() {
...
void Viewer_conic::render(const std::string &filename) {
  ...
  std::string data = read_file("../data/geogebra_conics_template.html");

  // remove name doublons
  remove_name_doublons();
  ...
  for (const auto &e : entries) {
    ...
    if ((e.m_show_label == true) && (e.m_show_value == true)) {
      commands +=
          "   api.evalCommand(\"ShowLabel(" + e._objectName + ", true)\");\n";
      commands +=
          "   api.evalCommand(\"SetLabelMode(" + e._objectName + ", 1)\");\n";
    }
    if ((e.m_show_label == true) && (e.m_show_value == false)) {
      commands +=
          "   api.evalCommand(\"ShowLabel(" + e._objectName + ", true)\");\n";
      commands +=
          "   api.evalCommand(\"SetLabelMode(" + e._objectName + ", 0)\");\n";
    }
    if ((e.m_show_label == false) && (e.m_show_value == true)) {
      commands +=
          "   api.evalCommand(\"ShowLabel(" + e._objectName + ", true)\");\n";
      commands +=
          "   api.evalCommand(\"SetLabelMode(" + e._objectName + ", 2)\");\n";
    }
    if ((e.m_show_label == false) && (e.m_show_value == false)) {
      commands +=
          "   api.evalCommand(\"ShowLabel(" + e._objectName + ", false)\");\n";
    }
  }
...
int Viewer_conic::push_point(const Eigen::VectorXd &pt, std::string objectName,
...
int Viewer_conic::push_line(const Eigen::VectorXd &pt,
                            const Eigen::VectorXd &dir, std::string objectName,
                            const unsigned int &red, const unsigned int &green,
                            const unsigned int &blue) {
  // remove space in the name
  objectName.erase(std::remove(objectName.begin(), objectName.end(), ' '),
...
// https://wiki.geogebra.org/en/Line_Command
int Viewer_conic::push_direction(const Eigen::VectorXd &direction,
